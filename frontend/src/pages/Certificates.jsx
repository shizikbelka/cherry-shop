...
const Certificates = () => {
    const certificates = products.filter(p => p.category_name === 'Сертификаты' || p.isCertificate === true);
            ...
            <div className="container mx-auto px-4 py-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                    {certificates.map((cert) => {
                        const isAdded = addedProductId === cert.id;
                        
                        return (
                            <Link 
                                key={cert.id} 
                                to={`/product/${cert.id}`}
                                className="bg-white rounded-xl shadow-md overflow-hidden card-hover flex flex-col transition-transform duration-300 hover:-translate-y-1"
                            >
                                ...
                                <div className="p-6 flex flex-col flex-grow">
                                    ...
                                    <p className="text-gray-500 text-sm mb-4 line-clamp-3">{cert.description}</p>
                                    
                                    <button
                                        onClick={(e) => handleAddToCart(cert, e)}
                                        className={`mt-auto py-3 rounded-lg transition-all duration-300 font-poppins flex items-center justify-center gap-2 ${
                                            isAdded 
